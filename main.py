    ...
    def mouseMoveEvent(self, event):
        if self._drag_start and (event.buttons() & Qt.MouseButton.LeftButton):
            ...
            if delta > 3:
                ...
                self.toolbar.move(event.globalPosition().toPoint() - self._offset)
            event.accept()
        else:
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            if not self._is_dragging:
                ...
                # Завершили перетаскивание — проверяем магнит или отскок от текста:
...
class FloatingToolbar(QFrame):
    ...
    def _get_dock_anchor_pos(self, anchor: str) -> QPoint:
        ...
        if anchor == "top_left":
            return QPoint(card.left(), card.top() - h - TOOLBAR_GAP)
        elif anchor == "bottom_left":
            return QPoint(card.left(), card.bottom() + TOOLBAR_GAP)
        elif anchor == "bottom_right":
            return QPoint(card.right() - w + 1, card.bottom() + TOOLBAR_GAP)
        else:  # top_right
            return QPoint(card.right() - w + 1, card.top() - h - TOOLBAR_GAP)
    ...
    def on_drag_finished(self):
        ...
        # 1. Если тулбар бросили ПРЯМО ПОВЕРХ ТЕКСТА — моментальный отскок к ближайшему углу!
...
class TranslateWindow(QWidget):
    ...
    def __init__(self, settings):
        ...
        self._is_paused = False  # ФЛАГ ЖЕЛЕЗНОЙ ПАУЗЫ
    ...
    def set_paused_mode(self, paused: bool):
        ...
        if paused:
            ...
            self.toolbar.show()
            self.toolbar.raise_()
            # Проверяем положение тулбара: если был прикреплен — синхронизируем,
            # если был оставлен над появившимся окном — мягко выталкиваем к ближайшему углу!
    ...
    def show_translation(self, text, x=None, y=None):
        ...
        # ЖЕЛЕЗНЫЙ ЗАПРЕТ ВСПЛЫВАТЬ НА ПАУЗЕ:
        if not self.isVisible() and not self.force_hidden and not self._is_paused:
            self.show()
